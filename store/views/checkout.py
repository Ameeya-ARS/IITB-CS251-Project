...
class Checkout(View):
    def post(self, request):
        ...
        for product in products:
            order = Order(customer = Customer(id = customer), product = product, quantity = cart.get(str(product.id)), price = product.price, address = address, address2 = address2, phone = phone, city = city, state = state, zip = zip)
            order.placeOrder()
